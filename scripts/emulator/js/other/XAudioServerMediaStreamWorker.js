...
self.onprocessmedia = function (event) {
	...
	const apiBufferLength = event.audioLength;
	const apiBufferLengthAll = apiBufferLength * event.audioChannels;
	...
	const sampleFramesCount = Math.min(apiBufferLength, XAudioJSResampledSamplesLeft() / XAudioJSChannelsAllocated);
